...
It may be too specific, resulting in the point estimate not ending up with one true number. Confidence intervals allow for a greater range and likelihood the true number is within the range.
...
I would be surprised if 40 of them were left handed, because there was a lot of variability between the last random sample and this one.
...
```{r data_manhattan}
manhattan <- read_csv("~/R/lecture-note-class-13-EleanorSeo/manhattan.csv")
```
...
```{r sample_iq_scores}
population %>% 
  slice_sample(n = 30) %>% # sample 30 data points
  summarise(mean_act = mean(act))
```

Did everyone get the same value? In fact, each time you sample (run the above
...
```{r set_seed}
set.seed(1902370) # allows us to have the same number / same random process
```
...
- Sample: 1-bedroom apartments in Manhattan
- Sample size: 20

To create our confidence interval, we'll follow the four-step bootstrap
scheme outlined above using functions from `infer`.

1. **Take a bootstrap sample** - a random sample taken with replacement 
   from the original sample, of the same size as the original sample. (Samples of 20 apartments)
...
For 95% of the time, in repeated samples, you would catch the true value inside an interval (x, y).
...
This result is similar to the previous confidence interval for mean price. However, using the median will be more robust as outliers will not be accounted for compared to if we were to use the mean.
...
We are 95% confident that the true proportion of mice that died while not on the treatment is between the interval 69-79.7%.
...
boot_prop_1 <- mice %>% 
  filter(treat == 1) %>% 
  mutate(lived = if_else(died == 0, "yes", "no")) %>% 
  specify(response = lived, success = "yes") %>% 
  generate(reps = 10000, type = "bootstrap") %>% 
  calculate(stat = "prop")
```

```{r practice_2_ci}
ci_mice_lived <- get_ci(boot_prop_1, level = .90)
ci_mice_lived
```
...
visualise(boot_prop_1) +
  shade_ci(ci_mice_lived)
```

Give an interpretation of your result.

We are 90% confident that the true proportion of mice that lived while on the treatment, regardless of the dose, is between 55.2-65.1%. The confidence interval is narrower for an interval of 0.90 compared to 0.95.
